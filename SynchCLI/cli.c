...
// CS307 - PA2 Synchronizing the CLI Simulator
...
// remove all the occurance of a character ('\r' carriage return problem, I explained this to TAs)
...
int main(int argc, char *argv[])
{
	...
	while (fgets(line, sizeof(line), fptr))
	{
		...
		{
			// if there is no output redirection create pipe and new thread
			// pointThread shows the current thread and thrCount counts the threads
			if (redirection != '>')
			{
				pointThread = thrCount;
				pipe(fds + pointThread * 2);
				pthread_create(&(threads[pointThread]), NULL, display, &fds[pointThread * 2]);
				thrCount++;
			}
